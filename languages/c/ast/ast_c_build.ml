...
let empty_env () =
  { struct_defs_toadd = []; enum_defs_toadd = []; typedefs_toadd = [] }
...
let debug any = Log.debug (fun m -> m "%s" (Ast_cpp.show_any any))
...
let rec ifdef_skipper xs f =
  ...
  | x :: xs -> (
      ...
      | Some ifdef -> (
          match ifdef with
          | Ifdef tok -> (
              Log.debug (fun m -> m "skipping: %s" (Tok.content_of_tok tok));
              try
                let _, x, rest =
                  xs
                  |> Common2.split_when (fun x ->
                         match f x with
                ...
                in
                match f x with
                | Some (IfdefEndif _) -> ifdef_skipper rest f
                | Some (IfdefElse _) ->
                    let before, _x, rest =
                      rest
                      |> Common2.split_when (fun x ->
  ...
  | DeclList (xs, _) ->
      ...
      @ (xs
        |> List_.map (fun x ->
               ...
               | _ -> A.VarDef x))
  (* todo *)
  | Asm (_tok1, _volatile_opt, _asmbody, _tok2) -> raise Todo
  | EmptyDef _ -> []
  | UsingDecl _
  | NamespaceAlias _
  | StaticAssert _
...
and parameter env x =
  match x with
  | P { p_name = n; p_type = t; p_val = v; p_specs = _TODO } ->
      (match v with
      | None -> ()
      | Some _ ->
          debug (Parameter x);
      ...
      let rec aux xs =
        ...
        | x :: xs -> (
            ...
            | X (S (Default (_, _, sts))) ->
                ...
                in
                (match x with
                | X (S (Case (tok, e, _, _))) -> A.Case (tok, expr env e, stmts)
                | X (S (Default (tok, _, _st))) -> A.Default (tok, stmts)
                | _ -> raise Impossible)
                :: aux rest
            | x ->
                debug (Body (l, [ x ], r));
...
and expr env e =
  ...
  | ConstructedObject (_, _) ->
      Log.err (fun m -> m "BUG PARSING LOCAL DECL PROBABLY");
      debug (Expr e);
      raise CplusplusConstruct
  | Throw _
  | Delete (_, _, _, _)
...
and argument env x =
  ...
  | ArgAction _ ->
      Log.warn (fun m -> m "type argument, maybe wrong typedef inference!");
      debug (Argument x);
      None
...
and full_type env x =
  ...
  | ClassName ((kind, _), n) -> A.TStructName (struct_kind env kind, name env n)
  | ClassDef (name_opt, def) -> (
      match def with
      | { c_kind = kind, tok; c_inherit = _inh; c_members = t1, xs, t2 } ->
          let name =
            match name_opt with
            | None ->
                incr cnt;
...
and fieldkind env decl =
  ...
  | TypedefDecl (_tk, _ty, _id) ->
      ...
      raise Todo
  | V
      ( { name = n; specs = _check_sto_emptyTODO },
        { v_init = _TODOcheckNone; v_type = ft } ) ->
      { A.fld_name = Some (name env n); fld_type = full_type env ft }
  | StructuredBinding _ -> raise CplusplusConstruct
  | BitField (name_opt, _tok, ft, e) ->
      let _TODO = expr env e in
      { A.fld_name = name_opt; fld_type = full_type env ft }
