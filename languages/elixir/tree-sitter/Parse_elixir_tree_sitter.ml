...
let mk_call_no_parens id_or_remote args blopt =
  ...
  | Either.Left id -> (I id, fb args, blopt)
  | Either.Right remote -> (DotRemote remote, fb args, blopt)
...
let map_quoted_xxx (env : env) (v1, v2, v3, v4) : string wrap bracket =
  let l = (* "/" or another one *) token env v1 in
  (* v2: optional first content token *)
  let first_parts = Option.to_list v2 |> List_.map (fun tok -> str env tok) in
  (* v3: list of (escape_sequence, optional_content) pairs *)
  let rest_parts =
    List.concat_map
      (fun (esc_tok, content_opt) ->
        let esc = str env esc_tok in
        let rest =
          Option.to_list content_opt |> List_.map (fun tok -> str env tok)
        in
        esc :: rest)
      v3
  in
  let r = (* "/" or another one *) token env v4 in
  G.string_ (l, first_parts @ rest_parts, r)

let map_quoted_i_xxx f_map_interpolation (env : env) (v1, v2, v3, v4) =
  let v1 = (* "<" or another one *) token env v1 in
  (* v2: optional first content token *)
  let first_parts =
    Option.to_list v2 |> List_.map (fun tok -> Either.Left (str env tok))
  in
  (* v3: list of (interp_or_escape, optional_content) pairs *)
  let rest_parts =
    List.concat_map
      (fun (interp_or_esc, content_opt) ->
        let sep_part =
          match interp_or_esc with
          | `Interp x ->
              let l, e, r = f_map_interpolation env x in
              Either.Right (l, e, r)
          | `Esc_seq tok -> Either.Left (str env tok)
        in
        let content_part =
          Option.to_list content_opt |> List_.map (fun tok -> Either.Left (str env tok))
        in
        sep_part :: content_part)
      v3
  in
  let v4 = (* ">" or another one *) token env v4 in
  (v1, first_parts @ rest_parts, v4)
...
let map_operator_identifier (env : env) (x : CST.operator_identifier) :
  ...
  | `HATHATHAT tok -> (* "^^^" *) (OOther "^^^", token env tok)
...
and map_atom (env : env) (x : CST.atom) : atom =
  ...
  | `Meta_atom (v1, v2) ->
      ...
      let s, tok = (* semgrep_metavariable *) str env v2 in
      (t, X1 (s, tok))
...
and map_binary_operator (env : env) (x : CST.binary_operator) : expr =
  ...
  | `Exp_DOTDOT_exp (v1, v2, v3) ->
      let v1 = map_expression env v1 in
      let v2 = (O G.Range, (* ".." *) token env v2) in
      let v3 = map_expression env v3 in
      binary_call v1 v2 v3
  ...
  | `Op_id_SLASH_int (v1, v2, v3) ->
      ...
      OpArity (op, tslash, Parsed_int.parse (s, t))
...
and map_call_without_parentheses (env : env) (x : CST.call_without_parentheses)
  ...
  | `Local_call_with_parens (v1, v2, v3) ->
      ...
      let blopt = map_anon_opt_opt_nl_before_do_do_blk_3eff85f env v3 in
      mk_call_no_parens (Either.Left id) args blopt
  | `Local_call_just_do_blk (v1, v2) ->
      let id = map_identifier env v1 in
      let bl = map_do_block env v2 in
      mk_call_no_parens (Either.Left id) ([], []) (Some bl)
  | `Remote_call_with_parens (v1, v2, v3) ->
      ...
      in
      let blopt = map_anon_opt_opt_nl_before_do_do_blk_3eff85f env v3 in
      mk_call_no_parens (Either.Right rdot) args blopt
...
and map_expression (env : env) (x : CST.expression) : expr =
  ...
  | `Sigil (v1, v2, v3) ->
      ...
      let sigil_kind =
        ...
        | `Imm_tok_pat_b250293_choice_quoted_double (v1, v2) ->
            let upper, t = (* pattern [A-Z][A-Z0-9]* *) str env v1 in
  ...
  | `Null_op tok ->
      (* ".." range with no bounds (Elixir 1.14+) *)
      let t = token env tok in
      I (Id ("..", t))
  ...
  | `Anon_func (v1, v2, v3, v4) ->
      ...
      let clauses =
        ...
        | None -> []
        | Some (x, xs) ->
            let x = map_stab_clause env x in
            let xs =
              List_.map
                (fun (v1, v2) ->
                  let _v1 = map_terminator env v1 in
...
and map_keyword (env : env) (x : CST.keyword) : keyword =
  ...
  | `Quoted_kw (v1, v2) ->
      ...
      (Quoted1 v1, tcolon)
  | `Meta_kw (v1, v2) ->
      let x = (* semgrep_metavariable *) str env v1 in
      let tcolon = (* pattern :\s *) token env v2 in
      (X1 x, tcolon)
...
and map_pair (env : env) (x : CST.pair) : pair =
  match x with
  | `Kw_exp (v1, v2) ->
      ...
      (v1, v2)
  | `Semg_ellips tok ->
      (* keyword ellipsis in semgrep patterns *)
      let t = token env tok in
      ((X1 ("...", t), t), I (IdEllipsis t))

and map_quoted_i_angle (env : env) ((v1, v2, v3, v4) : CST.quoted_i_angle) : quoted
    =
