...
(* Hook for type inference of expressions, set by osemgrep's Typing module.
 * This combines the old Typing.hook_type_of_expr + Type.to_ast_generic_type_
 * into a single step that directly returns an AST_generic.type_ option.
 *
 * TODO: we use a ref here instead of Hook.t because the hook needs to be
 * set once at initialization by Typing.ml, not scoped around a specific call.
 * We should try to reorganize the code so we can use a scoped Hook here.
 *)
let hook_type_of_expr :
...
let rec get_resolved_type lang (vinit, vtype) =
  ...
  | Some { t = TyAny _; _ } -> (
      ...
      *)
      let pro_type =
        match (!hook_type_of_expr, vinit) with
        | Some f, Some e -> f lang e
        | _ -> None
      in
      match pro_type with
      | Some x -> Some x
      ...
      method! visit_directive venv x =
        ...
        | ImportFrom (_, FileName (s, tok), imported_names) ->
            List.iter
              (function
                | Direct (id, id_info)
                  (* coupling: should be equivalent to Ast_js.default_entity *)
                  when Lang.is_js lang && fst id <> "default" ->
