...
let visit_program ~tag_hook _prefs _file (trees, toks) =
           ...
           (fun (k, _v) trees ->
             ...
             | F.Tok (s, _)
               ...
               :: _ ->
                 ...
                 k trees
             (* {...}{...} *)
             | F.Braces (_, brace_trees1, _)
               :: F.Braces (_, brace_trees2, _)
               :: _ ->
  ...
  |> List.iter (fun tok ->
         ...
         | T.TNowebChunkStr (_, ii) -> tag ii EmbededCode
         | T.TNowebCode (_, ii) -> tag ii EmbededCode
         | T.TNowebCodeLink (_, ii) -> tag ii (Label Def) (* TODO *)
         | T.TNowebChunkName (_, ii) -> tag ii KeywordObject (* TODO *)
         | T.TBeginVerbatim ii
         | T.TEndVerbatim ii ->
             tag ii Keyword
         | T.TVerbatimLine (_, ii) -> tag ii Verbatim
         (* syncweb-specific: *)
         | T.TFootnote (c, ii) -> (
             match c with
             | 't' -> tag ii BadSmell
             ...
             | _ ->
                 failwith (spf "syncweb \\x special macro not recognized:%c" c))
         | T.TNumber (_, ii)
         | T.TUnit (_, ii) ->
             if not (Hashtbl.mem already_tagged ii) then tag ii Number
         | T.TSymbol (s, ii) -> (
             match s with
             | "&"
             | "\\\\" ->
