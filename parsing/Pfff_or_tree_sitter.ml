...
(* claude: flags to restrict which parsers are used, adapted from Flag_semgrep *)
 ...
 fun parser file ->
  ...
  | Pfff f ->
      Common.save_excursion Flag_parsing.show_parsing_error false (fun () ->
          Logs.info (fun m -> m "trying to parse with Pfff parser %s" !!file);
          try
            let ast, stat = f file in
            ResOk (ast, stat, [])
          with
          | Exception.Timeout _ as e -> Exception.catch_and_reraise e
          | exn ->
              ...
              ResError e)
  | TreeSitter f -> (
      Logs.info (fun m -> m "trying to parse with TreeSitter parser %s" !!file);
      try
        let res = f file in
  ...
  | p :: xs -> (
      ...
      | ResPartial _ as partial -> (
          ...
          | ResError e2 ->
              ...
              partial
          | ResPartial _ ->
              Logs.debug (fun m -> m "Partial again but return first Partial");
              partial)
      | ResError e1 -> (
          let res = run_either file xs in
          match res with
...
let run_parser_pat p str =
  let parse () =
    match p with
    | PfffPat f ->
        Logs.info (fun m -> m "trying to parse with Pfff parser the pattern");
        f str
    | TreeSitterPat f ->
        Logs.info (fun m ->
