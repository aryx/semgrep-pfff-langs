...
type place =
  | PlaceLocal
  | PlaceSameDir
  | PlaceExternal (* | ReallyExternal   | PlaceCloseHeader *)
...
type category =
  ...
  | EnumName of usedef (* ClassName of place ... *)
  (* special types *)
  | TypeVoid
  | TypeInt
  (* haskell *)
...
let default_highlighter_preferences =
  { show_type_error = false; show_local_global = true }
...
let legend_color_codes =
  ...
  \  - gray background: not parsed, no type information, or other tool \
   limitations\n\
...
let info_of_place _defplace = raise Todo
...
let info_of_entity_kind_and_usedef2 kind defkind =
  ...
  (* defs *)
  | E.Function, Def2 _ ->
      [ `FOREGROUND "gold"; `WEIGHT `BOLD; `STYLE `ITALIC; `SCALE `MEDIUM ]
  | E.Macro, Def2 _ ->
      [ `FOREGROUND "gold"; `WEIGHT `BOLD; `STYLE `ITALIC; `SCALE `MEDIUM ]
  | E.Global, Def2 _ ->
      [ `FOREGROUND "cyan"; `WEIGHT `BOLD; `STYLE `ITALIC; `SCALE `MEDIUM ]
  | E.Constant, Def2 _ ->
      [ `FOREGROUND "pink"; `WEIGHT `BOLD; `STYLE `ITALIC; `SCALE `MEDIUM ]
  | E.Method, Def2 _ -> [ `FOREGROUND "gold3"; `WEIGHT `BOLD; `SCALE `MEDIUM ]
  ...
  | _ ->
      failwith
        (spf "info_of_entity_kind_and_usedef2: missing case for '%s'"
...
let info_of_category = function
  ...
  | PointerCall -> [ `FOREGROUND "firebrick"; `WEIGHT `BOLD; `SCALE `XX_LARGE ]
  | ParameterRef -> [ `FOREGROUND "magenta" ]
  | CallByRef -> [ `FOREGROUND "orange"; `WEIGHT `BOLD; `SCALE `XX_LARGE ]
  ...
  (* entities *)
  | Entity (kind, defkind) -> info_of_entity_kind_and_usedef2 kind defkind
  | FunctionDecl _ ->
      [ `FOREGROUND "gold2"; `WEIGHT `BOLD; `STYLE `ITALIC; `SCALE `MEDIUM ]
  ...
  | StaticMethod (Def2 _) ->
      [ `FOREGROUND "gold3"; `WEIGHT `BOLD; `SCALE `MEDIUM ]
  | StaticMethod (Use2 _) ->
      [ `FOREGROUND "gold3"; `WEIGHT `BOLD; `SCALE `MEDIUM ]
